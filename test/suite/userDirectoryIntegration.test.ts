...
suite("Integration: User Directory Deployment Flow", () => {
  ...
  setup(() => {
    ...
    sandbox.stub(process, "env").value({ ...process.env, APPDATA: tempDir });
  });
  ...
  suite("Fresh install → user directory created → templates deployed", () => {
    test("Should create full directory structure on fresh install", async () => {
      ...
      const root = userDirectoryService.getStorageRoot();
      const globalRoot = userDirectoryService.getGlobalNexkitRoot();
      const projectRoot = userDirectoryService.getProjectNexkitRoot();
      ...
      for (const subdir of expectedSubdirs) {
        const globalPath = path.join(globalRoot, subdir);
        const projectPath = path.join(projectRoot, subdir);
        assert.ok(fs.existsSync(globalPath), `Global subdirectory '${subdir}' should exist`);
        assert.ok(fs.statSync(globalPath).isDirectory(), `Global '${subdir}' should be a directory`);
        assert.ok(fs.existsSync(projectPath), `Project subdirectory '${subdir}' should exist`);
        assert.ok(fs.statSync(projectPath).isDirectory(), `Project '${subdir}' should be a directory`);
      }
    });
    ...
    test("Should resolve install path to user directory in user mode", () => {
      ...
      const expectedRoot = userDirectoryService.getProjectNexkitRoot();
  ...
  suite("Edge cases", () => {
    test("Should handle read-only directory gracefully", async () => {
      ...
      assert.ok(fs.existsSync(path.join(userDirectoryService.getGlobalNexkitRoot(), "agents")));
    });

    test("Should produce consistent paths regardless of call order", () => {
      const root1 = userDirectoryService.getProjectNexkitRoot();
      const locations = userDirectoryService.getAbsoluteTemplateLocations();
      const root2 = userDirectoryService.getProjectNexkitRoot();
    ...
    test("Should not fail when user directory already fully exists", async () => {
      ...
      const globalRoot = userDirectoryService.getGlobalNexkitRoot();
      const projectRoot = userDirectoryService.getProjectNexkitRoot();
      assert.ok(fs.existsSync(path.join(globalRoot, "agents")));
      assert.ok(fs.existsSync(path.join(globalRoot, "hooks")));
      assert.ok(fs.existsSync(path.join(projectRoot, "agents")));
      assert.ok(fs.existsSync(path.join(projectRoot, "hooks")));
    });

    test("Should return backup dir under user nexkit root", () => {
      const workspaceRoot = "C:\\workspace";
      const backupDir = userDirectoryService.getUserBackupDir(workspaceRoot);
      const root = userDirectoryService.getProjectNexkitRoot(workspaceRoot);
