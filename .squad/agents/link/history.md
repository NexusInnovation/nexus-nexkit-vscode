...
- 2026-08-07: In `RecommendedSettingsConfigDeployer` unchanged-settings tests, pre-seed `chat.plugins.marketplaces` with `NexusInnovation/nexus-plugin-marketplace#main` before asserting no writes. The deployer intentionally bootstraps this setting when missing, so omitting it from setup yields a false negative.
