interface CollapseAllButtonProps {
  onClick: () => void;
}

export function CollapseAllButton({ onClick }: CollapseAllButtonProps) {
  return (
    <button class="collapse-all-btn" title="Collapse All" onClick={onClick}>
      <svg width="32px" height="32px" viewBox="0 0 76 76" xmlns="http://www.w3.org/2000/svg" version="1.1">
        <style>{`.icon-path { fill: var(--vscode-icon-foreground); }`}</style>
        <path
          class="icon-path"
          fill-opacity="1"
          stroke-linejoin="round"
          d="M 19,29L 47,29L 47,57L 19,57L 19,29 Z M 43,33L 23,33.0001L 23,53L 43,53L 43,33 Z M 39,41L 39,45L 27,45L 27,41L 39,41 Z M 24,24L 51.9999,24.0001L 51.9999,52L 48.9999,52.0001L 48.9999,27.0001L 24,27.0001L 24,24 Z M 54,47L 53.9999,22.0001L 29,22L 29,19L 57,19L 57,47L 54,47 Z "
        />
      </svg>
    </button>
  );
}
