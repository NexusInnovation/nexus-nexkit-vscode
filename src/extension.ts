...
async function checkRequiredMCPs(
  ...
  try {
    ...
    if (missing.length > 0) {
      ...
      const dismissed = StateManager.getMcpSetupDismissed();
      ...
      if (!dismissed) {
        ...
        if (result === "Install") {
          ...
          await StateManager.setMcpSetupDismissed(true);
        }
      }
    }
  } catch (error) {
...
/**
 * Check if extension was updated and prompt user to update templates
 */
async function checkExtensionVersionAndPromptTemplateUpdate(
  context: vscode.ExtensionContext
): Promise<void> {
  try {
    ...
    const currentVersion =
      vscode.extensions.getExtension("nexusinno.nexkit-vscode")?.packageJSON
        .version || "0.0.0";
    const lastKnownVersion = StateManager.getLastKnownVersion();
    const isInitialized = StateManager.getWorkspaceInitialized();
    const promptEnabled = config.get(
      "extension.promptTemplateUpdate",
      true
    ) as boolean;

    // Store current version for next time
    await StateManager.setLastKnownVersion(currentVersion);

    // Check if extension was updated and workspace is initialized
    if (
      lastKnownVersion &&
      ...
      console.log(
        `Extension updated from ${lastKnownVersion} to ${currentVersion}`
      );

      const result = await vscode.window.showInformationMessage(
        `Nexkit extension updated to v${currentVersion}! Update your workspace templates to get the latest prompts, instructions, and chatmodes?`,
        "Update Templates",
        "Later",
        "Don't Ask Again"
      );

      if (result === "Update Templates") {
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  // Initialize state manager and migrate old config values
  StateManager.initialize(context);
  await StateManager.migrateFromConfiguration();
        ...
        switch (message.command) {
          ...
          case "initProject":
            ...
            webviewView.webview.postMessage({
              ...
              isInitialized: StateManager.getWorkspaceInitialized(),
            });
            break;
  ...
  // Check if extension was updated and prompt for template update
  checkExtensionVersionAndPromptTemplateUpdate(context);
        ...
        try {
          ...
          await vscode.window.withProgress(
            ...
            async (progress) => {
              ...
              await StateManager.setWorkspaceInitialized(true);
              await StateManager.setWorkspaceLanguages(wizardResult.languages);
              await StateManager.setWorkspaceMcpServers(
      ...
      await telemetryService.trackCommandExecution(
        ...
        async () => {
          try {
            ...
            // Get stored state
            const languages = StateManager.getWorkspaceLanguages();
            const mcpServers = StateManager.getWorkspaceMcpServers();
            ...
            const confirm = await vscode.window.showWarningMessage(
              "Update Nexkit templates from extension bundle?\n\n" +
                "This will overwrite:\n" +
